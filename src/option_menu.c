...
static int ProcessInput_FourOptions(int selection);
...
static void DrawOptionMenuChoice(const u8 *text, u8 x, u8 y, u8 style)
{
    ...
    dst[i] = EOS;
    AddTextPrinterParameterized(WIN_OPTIONS, FONT_NORMAL, dst, x, y + 1, 0, NULL);
}
...
static void DrawChoices_TextSpeed(int selection, int y)
{
    ...
    DrawOptionMenuChoice(gText_TextSpeedFast, xMid + widthMid + gap, y, styles[2]);
    DrawOptionMenuChoice(gText_TextSpeedInstant, 198 - widthInst, y, styles[3]);
}
...
static void DrawChoices_BattleType(int selection, int y)
{
    ...
    DrawOptionMenuChoice(gText_OptionMenuHide, 104, y, styles[0]);
    DrawOptionMenuChoice(gText_OptionMenuShow, GetStringRightAlignXOffset(FONT_NORMAL, gText_OptionMenuShow, 198), y, styles[1]);
}
