...
static void BagMenu_ItemPrintCallback(u8 windowId, u32 itemIndex, u8 y)
{
    ...
    if (itemIndex != LIST_CANCEL)
    {
        ...
        if (gBagPosition.pocket == BERRIES_POCKET)
        {
            ...
            BagMenu_Print(windowId, 7, gStringVar7, offset, y, 0, 0, TEXT_SPEED_FF, COLORID_NORMAL);
        }
...
static void PrintItemQuantity(u8 windowId, s16 quantity)
{
    ...
    StringExpandPlaceholders(gStringVar7, gText_xVar1);
    AddTextPrinterParameterized(windowId, 1, gStringVar7, GetStringCenterAlignXOffset(1, gStringVar7, 0x28), 2, 0, 0);
}
...
static void PrintItemSoldAmount(int windowId, int numSold, int moneyEarned)
{
    ...
    StringExpandPlaceholders(gStringVar7, gText_xVar1);
    AddTextPrinterParameterized(windowId, 1, gStringVar7, 0, 1, TEXT_SPEED_FF, 0);
...
static void StartItemSwap(u8 taskId)
{
    ...
    FillWindowPixelBuffer(WIN_DESCRIPTION, PIXEL_FILL(0));
    BagMenu_Print(WIN_DESCRIPTION, 1, gStringVar7, 3, 1, 0, 0, 0, COLORID_NORMAL);
...
static void ItemMenu_Give(u8 taskId)
{
    ...
    else if (!ItemId_GetImportance(gSpecialVar_ItemId) && ItemId_GetPocket(gSpecialVar_ItemId) != POCKET_TM_HM)
    {
...
static void PrintItemCantBeHeld(u8 taskId)
{
    ...
    DisplayItemMessage(taskId, 1, gStringVar7, HandleErrorMessage);
}
...
static void Task_ItemContext_Sell(u8 taskId)
{
    ...
    if (ItemId_GetPrice(gSpecialVar_ItemId) == 0 || ItemId_GetPocket(gSpecialVar_ItemId) == POCKET_TM_HM)
    {
        ...
        DisplayItemMessage(taskId, 1, gStringVar7, CloseItemMessage);
    }
    else
    {
        tItemCount = 1;
        ...
        else
        {
            ...
            DisplayItemMessage(taskId, 1, gStringVar7, InitSellHowManyInput);
        }
    }
}
...
static void DisplaySellItemPriceAndConfirm(u8 taskId)
{
    ...
    DisplayItemMessage(taskId, 1, gStringVar7, AskSellItems);
}
...
static void ConfirmSell(u8 taskId)
{
    ...
    DisplayItemMessage(taskId, 1, gStringVar7, SellItem);
}
...
static void AddBagSortSubMenu(void)
{
    ...
    if ((gBagPosition.pocket + 1) == POCKET_KEY_ITEMS)
        PrintContextMenuItemGrid(BagMenu_AddWindow(2), 2, 3);
    else if (gBagMenu->contextMenuNumItems == 4)
...
static void ItemMenu_Cancel2(u8 taskId)
{
    ...
    DisplayItemMessage(taskId, 1, sText_TooManyRegistered, HandleErrorMessage);
}
