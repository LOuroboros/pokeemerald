...
static void BuyMenuPrintPriceInList(u8 windowId, u32 itemId, u8 y)
{
    ...
    if (itemId != LIST_CANCEL)
    {
        ...
        x = GetStringRightAlignXOffset(FONT_NARROW, gStringBuffer, 120);
        AddTextPrinterParameterized4(windowId, FONT_NARROW, x, y, 0, 0, sShopBuyMenuTextColors[COLORID_ITEM_LIST], TEXT_SKIP_DRAW, gStringBuffer);
    }
}
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (sMartInfo.martType == MART_TYPE_DECOR)
                        StringExpandPlaceholders(gStringBuffer, gText_Var1IsItThatllBeVar2);
                    else // MART_TYPE_DECOR2
                        StringExpandPlaceholders(gStringBuffer, gText_YouWantedVar1ThatllBeVar2);

                    BuyMenuDisplayMessage(taskId, gStringBuffer, BuyMenuConfirmPurchase);
                }
            }
            break;
        }
...
static void Task_BuyHowManyDialogueInit(u8 taskId)
{
    ...
    StringExpandPlaceholders(gStringBuffer, gText_InBagVar1);
    BuyMenuPrint(WIN_QUANTITY_IN_BAG, gStringBuffer, 0, 1, 0, COLORID_NORMAL);
    tItemCount = 1;
...
static void BuyMenuPrintItemQuantityAndPrice(u8 taskId)
{
    ...
    BuyMenuPrint(WIN_QUANTITY_PRICE, gStringBuffer, 0, 1, 0, COLORID_NORMAL);
}
