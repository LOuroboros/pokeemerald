...
{
    ConvertEasyChatWordsToString(gStringBuffer, words, 3, 2);
    if (GetStringWidth(FONT_NORMAL, gStringBuffer, -1) > 204u)
    {
        s32 i = 0;

        ConvertEasyChatWordsToString(gStringBuffer, words, 2, 3);
        while (gStringBuffer[i++] != CHAR_NEWLINE)
            ;
        while (gStringBuffer[i] != CHAR_NEWLINE)
            i++;

        gStringBuffer[i] = CHAR_PROMPT_SCROLL;
    }
}
