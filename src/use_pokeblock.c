...
static void AskUsePokeblock(void)
{
    ...
    StringCopy(gStringBuffer, stringBuffer);
    FillWindowPixelBuffer(WIN_TEXT, 17);
    DrawTextBorderOuter(WIN_TEXT, 151, 14);
    AddTextPrinterParameterized(WIN_TEXT, FONT_NORMAL, gStringBuffer, 0, 1, 0, NULL);
...
static void PrintFirstEnhancement(void)
{
    ...
    if (sInfo->condition < CONDITION_COUNT)
        BufferEnhancedText(gStringBuffer, sInfo->condition, sInfo->enhancements[sInfo->condition]);
    else
        BufferEnhancedText(gStringBuffer, sInfo->condition, 0);

    PrintMenuWindowText(gStringBuffer);
    PutWindowTilemap(WIN_TEXT);
    CopyWindowToVram(WIN_TEXT, COPYWIN_FULL);
}
