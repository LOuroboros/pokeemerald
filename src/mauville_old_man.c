...
void GenerateGiddyLine(void)
{
    ...
    if (giddy->randomWords[giddy->taleCounter] != EC_EMPTY_WORD)
    {
        ...
        stringPtr = CopyEasyChatWord(gStringVar11, giddy->randomWords[giddy->taleCounter]);
    ...
    {
        StringCopy(gStringVar11, sGiddyQuestions[giddy->questionList[giddy->questionNum++]]);
    }
...
static void Task_BardSong(u8 taskId)
{
    ...
    case BARD_STATE_INIT:
        PrepareSongText();
        DrawSongTextWindow(gStringVar11);
        task->tWordState = 0;
        task->tDelay = 0;
        task->tCharIndex = 0;
        task->tCurrWord = 0;
    ...
    {
        struct MauvilleManBard *bard = &gSaveBlock1Ptr->oldMan.bard;
        u8 *str = &gStringVar11[task->tCharIndex];
        u16 wordLen = 0;
    ...
    case BARD_STATE_PAUSE:
        ...
        break;
    case BARD_STATE_HANDLE_WORD:
        if (gStringVar11[task->tCharIndex] == EOS)
        {
            // End song
            FadeInBGM(6);
        ...
        else if (gStringVar11[task->tCharIndex] == CHAR_SPACE)
        {
            // Handle space
            EnableTextPrinters();
            task->tCharIndex++;
        ...
        else if (gStringVar11[task->tCharIndex] == CHAR_NEWLINE)
        {
            // Handle newline
            task->tCharIndex++;
            task->tState = BARD_STATE_GET_WORD;
        ...
        else if (gStringVar11[task->tCharIndex] == EXT_CTRL_CODE_BEGIN)
        {
            // Handle ctrl code
        ...
        else if (gStringVar11[task->tCharIndex] == CHAR_BARD_WORD_DELIMIT)
        {
            // Handle word boundary
            gStringVar11[task->tCharIndex] = CHAR_SPACE;  // Replace with a real space
            EnableTextPrinters();
            task->tCharIndex++;
            task->tDelay = 0;
        }
