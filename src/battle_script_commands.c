...
static void Cmd_waitmessage(void)
{
    if (gBattleControllerExecFlags == 0)
    {
        ...
        else
        {
            ...
            if (++gPauseCounterBattle >= toWait|| (gSaveBlock2Ptr->optionsBattleAnimSpeed && (JOY_NEW(A_BUTTON | B_BUTTON))))
            {
    ...
    case 0: // check if should receive exp at all
        if (GetBattlerSide(gBattlerFainted) != B_SIDE_OPPONENT || (gBattleTypeFlags &
             (BATTLE_TYPE_LINK
              ...
              | BATTLE_TYPE_FRONTIER
              | BATTLE_TYPE_SAFARI
              | BATTLE_TYPE_BATTLE_TOWER)) || FlagGet(FLAG_DISABLE_EXPERIENCE))
        {
...
static void Cmd_pause(void)
{
    if (gBattleControllerExecFlags == 0)
    {
        ...
        if (++gPauseCounterBattle >= value || (gSaveBlock2Ptr->optionsBattleAnimSpeed && (JOY_NEW(A_BUTTON | B_BUTTON))))
        {
            gPauseCounterBattle = 0;
            gBattlescriptCurrInstr += 3;
        }
