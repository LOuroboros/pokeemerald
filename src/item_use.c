...
static void DisplayCannotUseItemMessage(u8 taskId, bool8 isUsingRegisteredKeyItemOnField, const u8 *str)
{
    ...
    {
        if (!InBattlePyramid())
            DisplayItemMessage(taskId, 1, gStringVar7, BagMenu_InitListsMenu);
        else
...
static void Task_ShowTMHMContainedMessage(u8 taskId)
{
    if (JOY_NEW(A_BUTTON | B_BUTTON))
    {
        ...
        DisplayItemMessage(taskId, 1, gStringVar7, UseTMHMYesNo);
    }
}
...
void ItemUseInBattle_Escape(u8 taskId)
{
    ...
    if((gBattleTypeFlags & BATTLE_TYPE_TRAINER) == FALSE)
    {
        RemoveUsedItem();
        if (!InBattlePyramid())
            DisplayItemMessage(taskId, 1, gStringVar7, Task_FadeAndCloseBagMenu);
        else
            DisplayItemMessageInBattlePyramid(taskId, gStringVar7, CloseBattlePyramidBagAndSetCallback);
    }
    else
    {
        DisplayDadsAdviceCannotUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem);
