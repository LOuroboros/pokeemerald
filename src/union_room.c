...
static void GetAwaitingCommunicationText(u8 *dst, u8 activity)
{
    ...
    case ACTIVITY_CONTEST_TOUGH:
        // BUG: argument *dst isn't used, instead it always prints to gStringBuffer
        // not an issue in practice since Gamefreak never used any other arguments here besides gStringBuffer
    #ifndef BUGFIX
        StringExpandPlaceholders(gStringBuffer, sText_AwaitingCommunication);
    #else
...
static void Task_TryBecomeLinkLeader(u8 taskId)
{
    ...
    case LL_STATE_GET_AWAITING_PLAYERS_TEXT:
        ...
        if (GROUP_MIN(sPlayerActivityGroupSize) != 0)
        {
            if (data->playerCount > GROUP_MIN(sPlayerActivityGroupSize) - 1 && GROUP_MAX(sPlayerActivityGroupSize) != 0)
                StringExpandPlaceholders(gStringBuffer, sText_AwaitingLinkPressStart);
            else
                StringExpandPlaceholders(gStringBuffer, sText_AwaitingCommunication);
        }
        else
        {
            GetAwaitingCommunicationText(gStringBuffer, gPlayerCurrActivity);
        }
    ...
    case LL_STATE_PRINT_AWAITING_PLAYERS:
        if (PrintOnTextbox(&data->textState, gStringBuffer))
            ...
            if (data->joinRequestAnswer == RFU_STATUS_JOIN_GROUP_OK)
            {
                ...
                if (data->playerCount == GROUP_MAX(sPlayerActivityGroupSize))
                {
                    ...
                    else
                    {
                        ...
                        StringExpandPlaceholders(gStringBuffer, sText_AnOKWasSentToPlayer);
...
static bool8 Leader_SetStateIfMemberListChanged(struct WirelessLink_Leader *data, u32 joinedState, u32 droppedState)
{
    ...
    case UNION_ROOM_SPAWN_IN:
        ...
        Leader_GetAcceptNewMemberPrompt(gStringBuffer, gPlayerCurrActivity);
...
static void Task_TryJoinLinkGroup(u8 taskId)
{
        ...
        case 0:
            ...
            if (JOY_NEW(A_BUTTON) && id != LIST_NOTHING_CHOSEN)
            {
                ...
                if (data->playerList->players[id].groupScheduledAnim == UNION_ROOM_SPAWN_IN && !data->playerList->players[id].rfu.data.startedActivity)
                {
                    ...
                    {
                        StringCopy(gStringBuffer, sCantTransmitToTrainerTexts[readyStatus - 1]);
    ...
    case LG_STATE_ASK_JOIN_GROUP:
        GetYouAskedToJoinGroupPleaseWaitMessage(gStringBuffer, gPlayerCurrActivity);
        if (PrintOnTextbox(&data->textState, gStringBuffer))
        {
    ...
    case LG_STATE_MAIN:
        ...
        case RFU_STATUS_JOIN_GROUP_OK:
            GetGroupLeaderSentAnOKMessage(gStringBuffer, gPlayerCurrActivity);
            if (PrintOnTextbox(&data->textState, gStringBuffer))
            {
                ...
                else
                {
                    ...
                    StringCopy(gStringVar1, sLinkGroupActivityNameTexts[gPlayerCurrActivity]);
                    StringExpandPlaceholders(gStringBuffer, sText_AwaitingOtherMembers);
                }
            }
            break;
...
static void Task_SendMysteryGift(u8 taskId)
{
    ...
    case 2:
        ...
        break;
    case 3:
        AddTextPrinterToWindow1(gStringBuffer);
        data->state = 4;
        break;
    case 4:
    ...
    case 5:
        ...
        break;
    case 7:
        switch (DoMysteryGiftYesNo(&data->textState, &data->yesNoWindowId, FALSE, gStringBuffer))
        {
        case 0:
            LoadWirelessStatusIndicatorSpriteGfx();
            CreateWirelessStatusIndicatorSprite(0, 0);
...
static void ScheduleFieldMessageWithFollowupState(u32 nextState, const u8 *src)
{
    ...
    if (src != gStringBuffer)
        StringExpandPlaceholders(gStringBuffer, src);
}
...
static void Task_RunUnionRoom(u8 taskId)
{
    ...
    case UR_STATE_INTERACT_WITH_PLAYER:
        ...
        case 2: // Ask to join chat
            ScheduleFieldMessageWithFollowupState(UR_STATE_RECV_JOIN_CHAT_REQUEST, gStringBuffer);
            break;
        }
        break;
        ...
        if (input != LIST_NOTHING_CHOSEN)
        {
            ...
            else
            {
                ...
                if (input == LIST_CANCEL || input == IN_UNION_ROOM)
                {
                    ...
                    StringCopy(gStringBuffer, sIfYouWantToDoSomethingTexts[gLinkPlayers[0].gender]);
    ...
    case UR_STATE_TRAINER_APPEARS_BUSY:
        StringCopy(gStringBuffer, sText_TrainerBattleBusy);
    ...
    case UR_STATE_WAIT_FOR_RESPONSE_TO_REQUEST:
        if (!gReceivedRemoteLinkPlayers)
        {
            StringCopy(gStringBuffer, sText_TrainerBattleBusy); // Redundant, will be copied again in next state
            uroom->state = UR_STATE_TRAINER_APPEARS_BUSY;
        }
        else
        {
            ...
            if (uroom->partnerYesNoResponse == (ACTIVITY_ACCEPT | IN_UNION_ROOM))
            {
                ...
                {
                    ViewURoomPartnerTrainerCard(gStringBuffer, uroom, FALSE);
            ...
            {
                uroom->state = UR_STATE_REQUEST_DECLINED;
                GetURoomActivityRejectMsg(gStringBuffer, gPlayerCurrActivity | IN_UNION_ROOM, gLinkPlayers[0].gender);
    ...
    case UR_STATE_PRINT_CARD_INFO:
        if (PrintOnTextbox(&uroom->textState, gStringBuffer))
        {
    ...
    case UR_STATE_WAIT_FINISH_READING_CARD:
        if (IsLinkTaskFinished())
        {
            if (GetMultiplayerId() == 0)
            {
                ...
                StringExpandPlaceholders(gStringBuffer, sAwaitingResponseTexts[id]);
    ...
    case UR_STATE_WAIT_FOR_CONTACT_DATA:
        ...
        if (!FuncIsActiveTask(Task_ExchangeCards))
        {
            ...
            id = PlayerHasMetTrainerBefore(gLinkPlayers[1].trainerId, gLinkPlayers[1].name);
            StringExpandPlaceholders(gStringBuffer, sPlayerContactedYouTexts[id]);
        }
        break;
    ...
    case UR_STATE_HANDLE_CONTACT_DATA:
        ...
        if (HandleContactFromOtherPlayer(uroom) && JOY_NEW(B_BUTTON))
        {
            ...
            StringCopy(gStringBuffer, sText_ChatEnded);
            uroom->state = UR_STATE_CANCEL_REQUEST_PRINT_MSG;
        }
        break;
    case UR_STATE_RECV_ACTIVITY_REQUEST:
        ScheduleFieldMessageWithFollowupState(UR_STATE_HANDLE_ACTIVITY_REQUEST, gStringBuffer);
        break;
    case UR_STATE_HANDLE_ACTIVITY_REQUEST:
        ...
        case 0: // ACCEPT
            ...
            if (gPlayerCurrActivity == (ACTIVITY_BATTLE_SINGLE | IN_UNION_ROOM))
            {
                if (!HasAtLeastTwoMonsOfLevel30OrLower())
                {
                    ...
                    StringCopy(gStringBuffer, sText_NeedTwoMonsOfLevel30OrLower2);
                }
                else
                {
                    Rfu_SendPacket(uroom->playerSendBuffer);
            ...
            {
                Rfu_SendPacket(uroom->playerSendBuffer);
                ViewURoomPartnerTrainerCard(gStringBuffer, uroom, TRUE);
                uroom->state = UR_STATE_PRINT_CARD_INFO;
            }
            else
            {
        ...
        case MENU_B_PRESSED:
            ...
            Rfu_SendPacket(uroom->playerSendBuffer);
            uroom->state = UR_STATE_DECLINE_ACTIVITY_REQUEST;
            GetYouDeclinedTheOfferMessage(gStringBuffer, gPlayerCurrActivity);
            break;
        }
        break;
    ...
    case UR_STATE_CANCEL_REQUEST_PRINT_MSG:
        ...
        {
            gPlayerCurrActivity = IN_UNION_ROOM;
            ScheduleFieldMessageWithFollowupState(UR_STATE_CANCEL_REQUEST_RESTART_LINK, gStringBuffer);
    ...
    case UR_STATE_PRINT_START_ACTIVITY_MSG:
        GetURoomActivityStartMsg(gStringBuffer, gPlayerCurrActivity | IN_UNION_ROOM);
        ScheduleFieldMessageWithFollowupState(UR_STATE_START_ACTIVITY_LINK, gStringBuffer);
        break;
    ...
    case UR_STATE_INTERACT_WITH_ATTENDANT:
        ...
        else
        {
            ...
            {
                StringCopy(gStringBuffer, sText_CancelRegistrationOfEgg);
            }
            else
            {
                ...
                StringExpandPlaceholders(gStringBuffer, sText_CancelRegistrationOfMon);
            }
            ScheduleFieldMessageWithFollowupState(UR_STATE_CANCEL_REGISTRATION_PROMPT, gStringBuffer);
        }
        break;
...
static bool32 HandleContactFromOtherPlayer(struct WirelessLink_URoom *uroom)
{
    ...
    {
        s32 id = GetChatLeaderActionRequestMessage(gStringBuffer, gLinkPlayers[1].gender, &uroom->recvActivityRequest[0], uroom);
        if (id == 0) // Error
        {
            return TRUE;
        }
...
static bool8 PrintOnTextbox(u8 *textState, const u8 *str)
{
    ...
    case 0:
        LoadMessageBoxAndBorderGfx();
        DrawDialogueFrame(0, TRUE);
        StringExpandPlaceholders(gStringBuffer, str);
...
static void PrintGroupMemberOnWindow(u8 windowId, u8 x, u8 y, struct RfuPlayer *player, u8 colorIdx, u8 id)
{
    ...
    ConvertIntToDecimalStringN(gStringBuffer, id + 1, STR_CONV_MODE_LEADING_ZEROS, 2);
    StringAppend(gStringBuffer, sText_Colon);
    PrintUnionRoomText(windowId, FONT_NORMAL, gStringBuffer, x, y, UR_COLOR_DEFAULT);
    x += 18;
    activity = player->rfu.data.activity;
    if (player->groupScheduledAnim == UNION_ROOM_SPAWN_IN && !(activity & IN_UNION_ROOM))
    {
        CopyAndTranslatePlayerName(gStringBuffer, player);
        PrintUnionRoomText(windowId, FONT_NORMAL, gStringBuffer, x, y, colorIdx);
        ...
        StringCopy(gStringBuffer, sText_ID);
        StringAppend(gStringBuffer, trainerId);
        PrintUnionRoomText(windowId, FONT_NORMAL, gStringBuffer, GetStringRightAlignXOffset(FONT_NORMAL, gStringBuffer, 0x88), y, colorIdx);
    }
}
...
static void PrintGroupCandidateOnWindow(u8 windowId, u8 x, u8 y, struct RfuPlayer *player, u8 colorIdx, u8 id)
{
    ...
    if (player->groupScheduledAnim == UNION_ROOM_SPAWN_IN)
    {
        ...
        StringCopy(gStringBuffer, sText_ID);
        StringAppend(gStringBuffer, trainerId);
        PrintUnionRoomText(windowId, FONT_NORMAL, gStringBuffer, GetStringRightAlignXOffset(FONT_NORMAL, gStringBuffer, 0x68), y, colorIdx);
    }
}
...
static s32 UnionRoomGetPlayerInteractionResponse(struct RfuPlayerList *list, bool8 overrideGender, u8 playerIdx, u32 playerGender)
{
    ...
    if (!player->rfu.data.startedActivity && !overrideGender)
    {
        ...
        {
            StringExpandPlaceholders(gStringBuffer, sJoinChatTexts[metBefore][playerGender]);
            return 2;
        }
        else
        {
    ...
    else
    {
        ...
        {
        case ACTIVITY_BATTLE_SINGLE:
            StringExpandPlaceholders(gStringBuffer, sBattleReactionTexts[playerGender][Random() % ARRAY_COUNT(sBattleReactionTexts[0])]);
            break;
        case ACTIVITY_TRADE:
            StringExpandPlaceholders(gStringBuffer, sTradeReactionTexts[playerGender][Random() % 2]);
            break;
        case ACTIVITY_CHAT:
            StringExpandPlaceholders(gStringBuffer, sChatReactionTexts[playerGender][Random() % ARRAY_COUNT(sChatReactionTexts[0])]);
            break;
        case ACTIVITY_CARD:
            StringExpandPlaceholders(gStringBuffer, sTrainerCardReactionTexts[playerGender][Random() % ARRAY_COUNT(sTrainerCardReactionTexts[0])]);
            break;
        default:
            StringExpandPlaceholders(gStringBuffer, sText_TrainerAppearsBusy);
            break;
        }
        return 0;
    }
...
static void ViewURoomPartnerTrainerCard(u8 *unused, struct WirelessLink_URoom *data, bool8 isParent)
{
    ...
    StringCopy(gStringBuffer, data->trainerCardMsgStrBuffer);
    ...
    StringAppend(gStringBuffer, data->trainerCardMsgStrBuffer);
