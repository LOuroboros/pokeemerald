...
static void PrintLinkBattleWinsLossesDraws(struct LinkBattleRecord *records)
{
    ...
    StringExpandPlaceholders(gStringBuffer, gText_TotalRecordWLD);

    x = GetStringCenterAlignXOffset(FONT_NORMAL, gStringBuffer, 0xD0);
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL, gStringBuffer, x, 0x11, 0, NULL);
}
...
void ShowLinkBattleRecords(void)
{
    ...
    StringExpandPlaceholders(gStringBuffer, gText_PlayersBattleResults);

    x = GetStringCenterAlignXOffset(FONT_NORMAL, gStringBuffer, 208);
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL, gStringBuffer, x, 1, 0, NULL);
    PrintLinkBattleWinsLossesDraws(gSaveBlock1Ptr->linkBattleRecords.entries);

    StringExpandPlaceholders(gStringBuffer, gText_WinLoseDraw);
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL, gStringBuffer, 0, 41, 0, NULL);
