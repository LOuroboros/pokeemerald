...
static void PrintItemQuantity(u8 windowId, u32 itemIndex, u8 y)
{
    ...
    xAlign = GetStringRightAlignXOffset(FONT_NARROW, gStringBuffer, 119);
    PyramidBagPrint_Quantity(windowId, gStringBuffer, xAlign, y, 0, 0, TEXT_SKIP_DRAW, COLORID_DARK_GRAY);
}
...
//  case PYRAMIDBAG_LOC_PARTY:
    ...
    FillWindowPixelBuffer(WIN_INFO, PIXEL_FILL(0));
    PyramidBagPrint(WIN_INFO, gStringBuffer, 3, 0, 0, 1, 0, COLORID_DARK_GRAY);
...
static void ShowNumToToss(void)
{
    ...
    x = GetStringCenterAlignXOffset(FONT_NORMAL, gStringBuffer, 0x28);
    AddTextPrinterParameterized(WIN_TOSS_NUM, FONT_NORMAL, gStringBuffer, x, 2, 0, NULL);
}
...
static void ShowCantHoldMessage(u8 taskId)
{
    ...
    DisplayItemMessageInBattlePyramid(taskId, gStringBuffer, Task_WaitCloseErrorMessage);
}
