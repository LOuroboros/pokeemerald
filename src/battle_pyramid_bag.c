...
static void PrintItemQuantity(u8 windowId, u32 itemIndex, u8 y)
{
    ...
    xAlign = GetStringRightAlignXOffset(FONT_NARROW, gStringVar11, 119);
    PyramidBagPrint_Quantity(windowId, gStringVar11, xAlign, y, 0, 0, TEXT_SKIP_DRAW, COLORID_DARK_GRAY);
}
...
//  case PYRAMIDBAG_LOC_PARTY:
    ...
    FillWindowPixelBuffer(WIN_INFO, PIXEL_FILL(0));
    PyramidBagPrint(WIN_INFO, gStringVar11, 3, 0, 0, 1, 0, COLORID_DARK_GRAY);
...
static void ShowNumToToss(void)
{
    ...
    x = GetStringCenterAlignXOffset(FONT_NORMAL, gStringVar11, 0x28);
    AddTextPrinterParameterized(WIN_TOSS_NUM, FONT_NORMAL, gStringVar11, x, 2, 0, NULL);
}
...
static void ShowCantHoldMessage(u8 taskId)
{
    ...
    DisplayItemMessageInBattlePyramid(taskId, gStringVar11, Task_WaitCloseErrorMessage);
}
