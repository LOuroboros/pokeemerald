...
static void PrintItemQuantity(u8 windowId, u32 itemIndex, u8 y)
{
    ...
    xAlign = GetStringRightAlignXOffset(7, gStringVar7, 119);
    PrintOnWindow_Font7(windowId, gStringVar7, xAlign, y, 0, 0, TEXT_SPEED_FF, COLORID_DARK_GRAY);
}
...
//  case PYRAMIDBAG_LOC_PARTY:
    ...
    FillWindowPixelBuffer(WIN_INFO, PIXEL_FILL(0));
    PrintOnWindow_Font1(WIN_INFO, gStringVar7, 3, 0, 0, 1, 0, COLORID_DARK_GRAY);
...
static void ShowNumToToss(void)
{
    ...
    AddTextPrinterParameterized(WIN_TOSS_NUM, 1, gStringVar7, x, 2, 0, NULL);
}
