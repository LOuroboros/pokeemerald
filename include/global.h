...
struct SaveBlock2
{
    ...
    /*0x??*/ u32 gcnLinkFlags; // Read by Pokemon Colosseum/XD
...
struct MauvilleManBard
{
    ...
    /*0x??*/ u8 playerTrainerId[TRAINER_ID_LENGTH];
    /*0x??*/ bool8 hasChangedSong;
    /*0x??*/ u8 language;
}; /*size = 0x??*/
...
struct SaveBlock1
{
    ...
    /*0x???*/ struct ObjectEventTemplate objectEventTemplates[OBJECT_EVENT_TEMPLATES_COUNT];
    /*0x????*/ u8 flags[NUM_FLAG_BYTES];
    /*0x????*/ u16 vars[VARS_COUNT];
    /*0x????*/ u32 gameStats[NUM_GAME_STATS];
    /*0x????*/ struct BerryTree berryTrees[BERRY_TREES_COUNT];
    /*0x????*/ struct SecretBase secretBases[SECRET_BASES_COUNT];
    /*0x????*/ u8 playerRoomDecorations[DECOR_MAX_PLAYERS_HOUSE];
    /*0x????*/ u8 playerRoomDecorationPositions[DECOR_MAX_PLAYERS_HOUSE];
    /*0x????*/ u8 decorationDesks[10];
    /*0x????*/ u8 decorationChairs[10];
    /*0x????*/ u8 decorationPlants[10];
    /*0x????*/ u8 decorationOrnaments[30];
    /*0x????*/ u8 decorationMats[30];
    /*0x????*/ u8 decorationPosters[10];
    /*0x????*/ u8 decorationDolls[40];
    /*0x????*/ u8 decorationCushions[10];
    /*0x????*/ TVShow tvShows[TV_SHOWS_COUNT];
    /*0x????*/ PokeNews pokeNews[POKE_NEWS_COUNT];
    /*0x????*/ u16 outbreakPokemonSpecies;
    /*0x????*/ u8 outbreakLocationMapNum;
    /*0x????*/ u8 outbreakLocationMapGroup;
    /*0x????*/ u8 outbreakPokemonLevel;
    /*0x????*/ u16 outbreakPokemonMoves[MAX_MON_MOVES];
    /*0x????*/ u8 outbreakPokemonProbability;
    /*0x????*/ u16 outbreakDaysLeft;
    /*0x????*/ struct GabbyAndTyData gabbyAndTyData;
    /*0x????*/ u16 easyChatProfile[EASY_CHAT_BATTLE_WORDS_COUNT];
    /*0x????*/ u16 easyChatBattleStart[EASY_CHAT_BATTLE_WORDS_COUNT];
    /*0x????*/ u16 easyChatBattleWon[EASY_CHAT_BATTLE_WORDS_COUNT];
    /*0x????*/ u16 easyChatBattleLost[EASY_CHAT_BATTLE_WORDS_COUNT];
    /*0x????*/ struct MailStruct mail[MAIL_COUNT];
    /*0x????*/ u8 additionalPhrases[8]; // bitfield for 33 additional phrases in easy chat system
    /*0x????*/ OldMan oldMan;
    /*0x????*/ struct DewfordTrend dewfordTrends[SAVED_TRENDS_COUNT];
    /*0x????*/ struct ContestWinner contestWinners[NUM_CONTEST_WINNERS]; // see CONTEST_WINNER_*
